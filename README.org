...
7. *raylib, and Flappy Bird* — a second renderer, as its own example:
   ...
   0–4 met by 2026-09-01 — their =flappy_bird.cljc=, byte-identical, playing
   on the iPhone 16 Pro. Left: the device numbers (milestone 5), and the
   simulator, which can no longer display OpenGL ES at all.
