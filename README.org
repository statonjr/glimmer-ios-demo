...
{:paths ["src"]
 ...
 :tasks {tangle "emacs --batch -l package --eval '(package-initialize)' -l org README.org -f org-babel-tangle </dev/null"
         ;; Milestone 1: cross-compile hello.core into the simulator bundle
         ;; (Info.plist is tangled), install it on the booted device, run it
         ;; with stdout attached. The pack is ~/dev/pack-tarm64ios-sim.
         bundle  "PACK=$HOME/dev/pack-tarm64ios-sim; SDK=$(xcrun -sdk iphonesimulator --show-sdk-path); mkdir -p Hello.app && JOLT_TARGET_CC=clang JOLT_TARGET_ARCH_FLAG=\"-target arm64-apple-ios-simulator -isysroot $SDK\" JOLT_TARGET_LINK_LIBS=\"-L$PACK/lib -llz4 -lz -framework Foundation -liconv -lm\" jolt build -m hello.core -o Hello.app/Hello --target tarm64ios --target-pack $PACK && rm -rf Hello.app/Hello.build"
         install "xcrun simctl install booted Hello.app"
         ;; not `run`: that is a jolt subcommand, and it wins over a task of the same name
         launch  "xcrun simctl launch --console booted dev.statonsoftware.glimmer-ios.hello"}}
#+end_src
