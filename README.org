...
* What it does, so far (2026-09-01)

Two days in, on an iPhone 16 Pro, on portable bytecode with no JIT. Each
picture links to the notebook that built it; the full-size media sit beside
these in =media/=.

** Flappy Bird

The Android experiment's pure =flappy_bird.cljc=, byte-identical, under a
thirty-line iOS owner loop over raylib 6.0 and SDL2, at 60 fps — recorded on
the phone (=media/flappy.mp4= is the full-resolution video).
→ [[file:examples/flappy/README.org][examples/flappy]]

[[file:media/flappy-small.gif]]

** The gallery

The Android experiment's scene contract, cards and input normalisation,
byte-identical, under one iOS host — with Following Eyes and Touch Trail
alongside the bird. Tap a card; Back returns.
→ [[file:examples/gallery/README.org][examples/gallery]]

[[file:media/gallery-small.png]] [[file:media/eyes-small.png]] [[file:media/trail-small.png]]

** raylib's first frame

Its default font and a =DrawCircle=, drawn by a jolt =while= loop inside
=UIApplicationMain=. The simulator can no longer display OpenGL ES at all, so
every raylib picture in these notebooks is hardware.
→ [[file:examples/flappy/README.org][examples/flappy]], milestone 2

[[file:media/raylib-small.png]]

** The counter

The macOS demo's view, character for character, over the glimmer seam on
UIKit (=media/counter.mp4= for Slack and the web).
→ [[file:examples/counter/README.org][examples/counter]]

[[file:media/counter-small.gif]]

** The app as data, over the wire

The same shell, its UI fetched as EDN from a server and negotiated by
=Accept-Language= — French here, because the phone asked. Labels travel; the
action vocabulary stays in code.
→ [[file:examples/counter/README.org][examples/counter]], "The app as data" and "Content negotiation"

[[file:media/wire-fr-small.png]]

Everything above is tangled from these notebooks: the counter and the wire
from [[file:examples/counter/README.org][examples/counter]], the bird from [[file:examples/flappy/README.org][examples/flappy]], the backend from
[[file:glimmer-ios/core.org][glimmer-ios/]]. The rest of this file is how it got there, in order,
mistakes included.
...
brew "cmake"                # cross-builds SDL2 and raylib for the simulator and the phone (examples/flappy, milestone 1)
#+end_src
...
{;; ffi/write took (p t offset v) before 0.8.0 and (p t v offset) after, and the
 ;; two cannot be told apart at runtime — so an older jolt must refuse, not
 ;; guess. Every struct here is a layout + write-field regardless (see "jolt
 ;; 0.8.0: the write order"); the floor is belt and braces.
...
<dict>
  ...
  <key>CFBundleDisplayName</key>       <string>Gallery</string>   <!-- the tile's label (was Counter until examples/gallery); the bundle stays Hello.app -->
...
Dmitri flagged the [[https://github.com/jolt-lang/jolt/blob/main/CHANGELOG.md#changed][0.8.0 CHANGELOG]] the day after it shipped, and it has a
breaking change aimed squarely at this notebook: =ffi/write= now takes the
value /before/ the offset — =(write p t v offset)=, babashka.ffi's order — where
0.7.x took =(write p t offset v)=. Both arguments are integers, so the old
spelling does not fail on the new runtime; it writes the wrong byte to the
wrong place, silently. This machine still runs 0.7.29 (the tap has not moved),
which is exactly the window in which to fix it, before an upgrade turns five
namespaces into heisenbugs.

The fix is not to swap arguments — that would break on 0.7.29 the way the old
order breaks on 0.8.0 — but to stop spelling structs as offsets at all.
=ffi/layout= and =ffi/write-field= are unchanged in both versions, so every raw
=write= in the tree became a named field of a layout, and every field is
written so nothing depends on 0.8.0's other change, zeroed =alloc=:

- =demo.repl= — =sockaddr_in= as nine fields; the =setsockopt= flag as a
  one-field struct.
- =demo.http= — the Block ABI as two layouts (=block-l=, =descriptor-l=), and
  =layout-size= instead of the literal 32.
- =glimmer-ios.core= — =CFRunLoopSourceContext= as ten pointer-sized fields
  instead of nine zeroed offsets and a =perform= at 72.
- =raylib.host= — only =SDL_SysWMinfo='s version bytes, through a three-field
  layout; SDL writes the rest.
- =raylib.window=, the superseded spike, is =:tangle no= now and keeps its raw
  writes as the record of how it was.

Verified after the rewrite, still on 0.7.29: the simulator's wire app
listened on its layout-built =sockaddr= (nREPL answered), fetched through the
layout-built block (=wire: rendered=) and redrew through the layout-built
run-loop source; the phone's Flappy Bird found SDL's framebuffer (=fbo 1=)
through the three-field version layout. Same bytes in memory, spelled by name.

** Upgraded (2026-09-01, later)

The tap moved the same day. =brew upgrade jolt=, =jolt --version= → 0.8.0,
and the checklist: =jolt tangle= and =jolt test= (15 / 60) unchanged; the
simulator's wire app listened on its layout-built =sockaddr=, fetched through
the layout-built block and redrew (=wire: rendered=); Flappy Bird on the phone,
rebuilt on 0.8.0, played — which is the host's version layout finding SDL's
framebuffer, since without it the screen is black. =:jolt/min-version "0.8.0"=
is in =deps.edn= now: an older jolt refuses rather than guesses.

One hope did not survive the probe. The simulator still reports =os.name=
"Linux", and =(jolt.nrepl/start 7999)= — tried live, over =demo.repl='s own
listener — still dies with =socket() failed=. Reading the CHANGELOG line again,
"a *portable-bytecode* build reported itself as Linux" is exactly what it
says: =tpb64l= is fixed, =tarm64ios= is not — a native machine type that is
neither =*osx= nor =*le= still falls through =sa-os-family=. So =demo.repl=
keeps its socket on the simulator, and the upstream note below stands for
the native iOS machine type. Whether the phone now says something other
than "Linux" is a =hello.core= run away, and worth its own line when taken.
...
Three other 0.8.0 lines matter here. *A portable-bytecode build reported
itself as Linux, wherever it was running* — that is milestone 1's =os.name=
finding, filed from this notebook and fixed; with it, =sa-arch= and
=sa-endian= answer for =pb=. *=:bool=, a one-byte C boolean* — the =:uint8=
that milestone 3's spike learned to use for =BOOL= has a name now. And
*=:jolt/min-version=* — this project will declare =0.8.0= once Homebrew
carries it; until then the code above runs correctly on either.

* App Store rule 2.5.2, verbatim (2026-09-01)

This notebook has cited 2.5.2 twice from memory — in the W^X section ("permits
interpreting code bundled with the app") and in [[file:examples/counter/README.org][examples/counter]]'s proof of
the app as data. Before anything gets posted on the strength of it, the text
itself, from [[https://developer.apple.com/app-store/review/guidelines/][developer.apple.com/app-store/review/guidelines]] as fetched today:
...
Two corrections to the memory. It does not say "permits interpreting"; it
says /self-contained/ and /may not download, install, or execute code/. And
the clause many people remember — "the only exception is code run by Apple's
built-in WebKit framework or JavaScriptCore, provided it doesn't provide
access to native APIs" — is no longer in it. That clause was what
React-Native-style over-the-air JavaScript stood on, and its absence is why
the wire's argument must not be "it is like JavaScript."

The argument is the one the code already makes. The bundle is self-contained:
the interpreter, the widget set and the action vocabulary all shipped through
review. What arrives over the wire is data — EDN, read by =clojure.edn=,
validated by spec against a closed set of kinds — and nothing in it can
extend what the app is able to do, only choose among things it already could:
a news app downloading articles, a browser downloading a page. The
demonstration's own line holds: nothing here can turn EDN into code. The one
feature that /does/ execute downloaded code is the nREPL, which is why every
notebook calls it dev-only; a shipped build would carry no listener.

That is a reading, not a ruling — the line between data that changes what an
app shows and code that changes what it does is exactly the one 2.5.2 leaves
to a reviewer — and "self-contained, no new capabilities" is the strongest
ground the rule offers to stand on.
...
- *jolt: =os.name= and =os.arch= on iOS machine types* — =sys-os-name=
  ...
  (Fixed in jolt 0.8.0 — "a portable-bytecode build reported itself as
  Linux, wherever it was running", 2026-08-31.)
...
From [[file:examples/flappy/README.org][examples/flappy]] (2026-09-01):

- *raylib: the SDL platform never binds SDL's drawable framebuffer* — iOS
  has no framebuffer 0; SDL's =README-ios= requires =uikit.framebuffer= bound
  while rendering and =uikit.colorbuffer= at swap, both from =SDL_SysWMinfo=.
  =rcore_desktop_sdl.c= draws into 0, which the simulator silently accepts and
  a device would not. One =SDL_GetWindowWMInfo= after =SDL_GL_CreateContext=,
  and a bind per frame, is the whole fix. Casualty: milestone 2's black screen.
- *raylib: =CUSTOMIZE_BUILD= reads =#define SUPPORT_X 0= as ON* — the
  =config.h= parser behind it keys on the =#define=, not the value, so
  =SUPPORT_CUSTOM_FRAME_CONTROL= and =SUPPORT_BUSY_WAIT_LOOP= (and a dozen
  file formats) flip on for anyone who customises anything. Casualty: an
  =EndDrawing= that swapped, timed and polled nothing.
- *raylib: =GetTouchPointId= truncates =SDL_FingerID=* — a 64-bit,
  pointer-shaped value on iOS, stored in an =int=; every finger reports
  =1211247552=. Cosmetic until someone needs multi-touch.
- *raylib: =FLAG_WINDOW_HIGHDPI= "is not supported on PLATFORM_DESKTOP_SDL"* —
  it still asks SDL for a high-DPI drawable, then sets =render = screen=. The
  notebook sizes the screen in pixels by hand; the platform could do the same
  from =SDL_GL_GetDrawableSize=.
- *Apple: the iOS 17.5+ simulator does not display OpenGL ES* — pixels reach
  the framebuffer (=glReadPixels= says so) and =CAEAGLLayer= shows black;
  hardware is fine. Known ([[https://developer.apple.com/forums/thread/803483][forums thread 803483]]); not a bug anyone will fix.
...
7. *raylib, and Flappy Bird* — a second renderer, as its own example:
   [[file:examples/flappy/README.org][examples/flappy]]. The Android experiment's pure =flappy_bird.cljc=
   unchanged, an iOS owner loop underneath; raylib static in the pack, SDL as
   the platform layer raylib lacks. Research written 2026-08-31; milestones
   0–5 met by 2026-09-01 — their =flappy_bird.cljc=, byte-identical, playing
   on the iPhone 16 Pro at a vsync-bound 17 ms. The simulator can no longer
   display OpenGL ES at all. Then [[file:examples/gallery/README.org][examples/gallery]]: their scene contract, cards,
   input normalisation and two more scenes (Following Eyes, Touch Trail),
   all unchanged, under one iOS host — the app's tile says Gallery now.
...
(2026-08-31, later: the second UI shape got onto the list after all — as
item 7, and as a separate example rather than a change to this one. The
gesture layer stays off it.)
