...
Two days in, on an iPhone 16 Pro, on portable bytecode with no JIT:

#+caption: Flappy Bird — the Android experiment's pure =flappy_bird.cljc=, byte-identical, under a thirty-line iOS owner loop over raylib 6.0 and SDL2, at 60 fps. Recorded on the phone; =media/flappy.mp4= is the full-resolution video. ([[file:examples/flappy/README.org][examples/flappy]])
[[file:media/flappy.gif]]

#+caption: raylib's first frame on the phone: its default font and a =DrawCircle=, drawn by a jolt =while= loop inside =UIApplicationMain=. The simulator can no longer display OpenGL ES at all, so every raylib picture here is hardware.
[[file:media/raylib.png]]

#+caption: The counter — the macOS demo's view, character for character, over the glimmer seam on UIKit. (=media/counter.mp4= for Slack and the web.) ([[file:examples/counter/README.org][examples/counter]])
[[file:media/counter.gif]]

#+caption: The app as data: the same shell, its UI fetched as EDN from a server and negotiated by =Accept-Language= — French here, because the phone asked. Labels travel; the action vocabulary stays in code.
