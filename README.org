...
media/*.mov
# the signing stub's generated project and build products (signing/project.yml is the source)
signing/*.xcodeproj
signing/build
#+end_src
...
{:paths ["src"]
 ...
 :tasks {;; every notebook, not just this one; never the deps checkout, the bundle, or docs/
         ...
         ;; iOS suspends a backgrounded app and its listener refuses; this brings it back
         ;; to the foreground (works over Wi-Fi — no cable) so `jolt connect` answers again
         wake    "xcrun devicectl device process launch --terminate-existing --device ${DEVICE:?set DEVICE to the phone UDID} dev.statonsoftware.glimmer-ios.hello"
...
2. *Lifecycle* — background/foreground the counter (=simctl= can do both),
   relaunch, an allocation stress like EXP-005. What does a parked Chez
   thread do on =applicationDidEnterBackground:=?
3. *Persistence* — a =:storage/write= effect to the Documents dir, restored on
   a fresh launch (EXP-012's shape); the first effect-as-data on this side.
